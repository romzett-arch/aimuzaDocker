-- ═══════════════════════════════════════════════════════════════
-- AUDIT FIXES: Security, Performance, Logic
-- P0: Block check in award_xp, RLS fix on economy tables
-- P1: Ban filter in feed
-- P2: Missing indexes
-- ═══════════════════════════════════════════════════════════════

-- ─── P0: Add is_user_blocked() check to award_xp ─────────────
-- Blocked users must NOT earn XP or reputation.
...
-- ─── P0: Fix RLS on economy tables ──────────────────────────
-- Replace profiles.role check with user_roles-based is_admin()

-- Drop and recreate broken policies
DROP POLICY IF EXISTS "economy_snapshots_read" ON public.economy_snapshots;
DROP POLICY IF EXISTS "economy_config_update" ON public.economy_config;
DROP POLICY IF EXISTS "attribution_shares_read" ON public.attribution_shares;
DROP POLICY IF EXISTS "creator_earnings_read" ON public.creator_earnings;

-- Recreate with proper is_admin() check
CREATE POLICY "economy_snapshots_read" ON public.economy_snapshots
  FOR SELECT TO authenticated
  USING (public.is_admin(auth.uid()));

CREATE POLICY "economy_config_update" ON public.economy_config
  FOR UPDATE TO authenticated
  USING (public.is_admin(auth.uid()));

CREATE POLICY "attribution_shares_read" ON public.attribution_shares
  FOR SELECT TO authenticated
...
-- ─── P2: Missing indexes for performance ────────────────────

-- reputation_events: composite for filtered queries in award_xp
CREATE INDEX IF NOT EXISTS idx_rep_events_user_type_date
  ON public.reputation_events (user_id, event_type, created_at DESC);

-- reputation_events: weekly/monthly aggregation in award_xp
CREATE INDEX IF NOT EXISTS idx_rep_events_user_xp_date
  ON public.reputation_events (user_id, created_at)
  WHERE xp_delta > 0;

-- tracks: admin moderation queries
CREATE INDEX IF NOT EXISTS idx_tracks_mod_status_date
  ON public.tracks (moderation_status, created_at DESC);

-- tracks: user's moderation status
CREATE INDEX IF NOT EXISTS idx_tracks_user_mod
  ON public.tracks (user_id, moderation_status);

-- balance_transactions: user history sorted by date
CREATE INDEX IF NOT EXISTS idx_balance_tx_user_date
  ON public.balance_transactions (user_id, created_at DESC);

-- payments: user history sorted by date
CREATE INDEX IF NOT EXISTS idx_payments_user_date
  ON public.payments (user_id, created_at DESC);

-- user_blocks: check blocks quickly
CREATE INDEX IF NOT EXISTS idx_user_blocks_user
  ON public.user_blocks (user_id);

-- ─── P1: Filter blocked users from feed ─────────────────────
-- Patch get_feed_tracks_with_profiles to exclude blocked users' tracks

CREATE OR REPLACE FUNCTION public.get_feed_tracks_with_profiles(
  p_user_id uuid DEFAULT NULL,
  p_tab text DEFAULT 'new',
...
RETURNS TABLE (
  ...
  likes_count integer, plays_count integer, status text, created_at timestamptz,
  profile_username text, profile_avatar_url text, genre_name_ru text
) AS $$
BEGIN
  RETURN QUERY
