-- Reconcile the support and QA foundation on installations that predate
-- the versioned support/QA migration chain.
...
CREATE UNIQUE INDEX IF NOT EXISTS qa_tickets_ticket_number_key
  ON public.qa_tickets(ticket_number)
...
BEGIN
  IF NEW.ticket_number IS NULL OR BTRIM(NEW.ticket_number) = '' THEN
    NEW.ticket_number := 'TKT-' || TO_CHAR(clock_timestamp(), 'YYYYMMDD') || '-'
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.qa_tickets TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.qa_comments TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.qa_votes TO authenticated;
GRANT SELECT ON public.qa_tester_stats, public.qa_bounties TO authenticated;
