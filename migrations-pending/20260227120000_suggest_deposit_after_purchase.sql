-- Обновить уведомление покупателю при одобрении сделки с текстом
-- Добавить рекомендацию депонировать + metadata для навигации
...
DECLARE
  ...
  v_item_title TEXT;
  v_buyer_notification_message TEXT;
  v_buyer_notification_metadata JSONB;
BEGIN
  ...
  -- Передать владение текстом покупателю (только эксклюзив)
  ...
  -- Уведомление покупателю с рекомендацией депонировать (для текстов)
  IF v_purchase.item_type = 'lyrics' THEN
    v_buyer_notification_message := v_item_title || ' — теперь в разделе «Мои тексты». Рекомендуем депонировать текст для защиты авторских прав.';
    v_buyer_notification_metadata := jsonb_build_object(
      'item_type', v_purchase.item_type,
      'source_id', v_purchase.source_id
    );
  ELSE
    v_buyer_notification_message := v_item_title || ' — покупка одобрена';
    v_buyer_notification_metadata := jsonb_build_object(
      'item_type', v_purchase.item_type
    );
  END IF;

  INSERT INTO public.notifications (user_id, type, title, message, actor_id, target_type, target_id, metadata)
  VALUES (
    v_purchase.buyer_id,
    'deal_approved',
    'Покупка одобрена',
    v_buyer_notification_message,
    auth.uid(),
    'item_purchase',
    p_purchase_id,
    v_buyer_notification_metadata
  );
END;
$$;
