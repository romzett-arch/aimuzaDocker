-- Защита от повторного применения pending-миграций.
...
'История SQL-файлов из deploy/migrations-pending, чтобы не применять одну и ту же миграцию повторно.';

-- Полное пересоздание resolve_qa_ticket без текстовых replace,
-- чтобы повторные прогоны миграций не могли дублировать xp_social.
...
BEGIN
  ...
  SELECT reporter_id, status, bounty_id, severity
    INTO v_reporter_id, v_old_status, v_bounty_id, v_severity
  ...
  -- Fallback: если заданы только рубли, взять XP из конфигурации по severity.
  ...
  SET status = p_status,
      ...
      resolved_by = CASE
        WHEN p_status IN ('fixed', 'wont_fix', 'closed') THEN v_admin_id
        ELSE resolved_by
      END,
      resolved_at = CASE
        WHEN p_status IN ('fixed', 'wont_fix', 'closed') THEN now()
        ELSE resolved_at
      END,
      reward_xp = CASE
        WHEN v_final_xp > 0 THEN v_final_xp
        ELSE reward_xp
      END,
      reward_credits = CASE
        WHEN v_final_credits > 0 THEN v_final_credits
        ELSE reward_credits
      END
  WHERE id = p_ticket_id;
  ...
  IF (v_final_xp > 0 OR v_final_credits > 0) AND NOT public.is_user_blocked(v_reporter_id) THEN
    ...
    IF v_final_credits > 0 THEN
      ...
      VALUES (
        ...
        v_final_credits,
        'qa_reward',
        'Награда за найденную ошибку #' || LEFT(p_ticket_id::TEXT, 8),
        'qa_ticket',
        p_ticket_id
      );
    END IF;
