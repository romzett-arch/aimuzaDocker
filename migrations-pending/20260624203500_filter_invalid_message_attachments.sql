-- Ignore empty or malformed attachment payload entries in messaging_send_message.
...
BEGIN
  ...
  WITH normalized AS (
    SELECT CASE jsonb_typeof(p_attachments)
  ...
  valid_entries AS (
    SELECT elem
    FROM normalized n,
         LATERAL jsonb_array_elements(n.payload) elem
    WHERE NULLIF(elem->>'storage_path', '') IS NOT NULL
  )
