-- Unified mass delivery for admin communications.
...
BEGIN
  ...
  IF p_popup_mode NOT IN ('modal', 'banner') THEN
    RAISE EXCEPTION 'Неизвестный вид всплывающего уведомления';
  END IF;
  ...
  IF 'popup' = ANY(p_channels) THEN
    ...
    VALUES (
      ...
      btrim(p_content),
      'news',
      p_popup_mode::public.announcement_display_mode,
      'info',
      true,
      true,
      now(),
...
REVOKE ALL ON FUNCTION public.admin_send_mass_broadcast(uuid[], text[], text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_send_mass_broadcast(uuid[], text[], text, text, text, text) TO authenticated;
